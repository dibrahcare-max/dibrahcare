...
export default function MyBookings() {
  ...
  useEffect(() => {
    // تحقق من الجلسة واجلب الحجوزات عبر API محمي (بدون cache لضمان حالة محدّثة)
  ...
  const exportExcel = () => {
    const rows = [
      ...
      ...filtered.map(b => [
        ...
        b.start_time || '—',
        b.end_time || '—',
        (typeof b.price === 'number' && b.price > 0) ? b.price.toLocaleString('ar-SA') + ' ريال' : '—',
  ...
  const exportWord = () => {
    const rows = filtered.map(b => `
      <tr>
        ...
        <td>${(typeof b.price === 'number' && b.price > 0) ? b.price.toLocaleString('ar-SA') + ' ريال' : '—'}</td>
                    ...
                    <div style={{ padding: '18px 20px', flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                        {[
                          ...
                          ['💰 المبلغ', (typeof b.price === 'number' && b.price > 0) ? b.price.toLocaleString('ar-SA') + ' ريال' : '—'],
                        ].map(([k, v]) => (

  ...
  return (
    <div className="print-wrap">
      ...
      <div className="print-page">
        ...
        {/* بيانات المستفيدين المتعددين (سفر/مستشفى/عروس/زفاف/مراهق/ديني...) */}
        {serviceDetails?.type === 'multi' && Array.isArray(serviceDetails.beneficiaries) &&
          serviceDetails.beneficiaries.map((b: any, idx: number) => (
            <Section key={idx} title={serviceDetails.beneficiaries.length > 1 ? `بيانات المستفيد ${idx + 1}` : 'بيانات المستفيد'}>
              <Row label="الاسم" value={b.name} />
              {b.age && <Row label="العمر" value={`${b.age} سنة`} />}
              {b.national_id && <Row label="رقم الهوية" value={b.national_id} mono />}
              {b.phone && <Row label="رقم الجوال" value={b.phone} mono />}
              {b.recommendations && <Row label="توصيات وملاحظات" value={b.recommendations} multiline />}
            </Section>
          ))
        }

...
export async function POST(req: NextRequest) {
  try {
    ...
    const selectedPackage = String(body.package || '').trim()
    ...
    // ─── قيد الباقة: لو الكود مخصّص لباقة معينة ───
    if (data.applies_to_package && data.applies_to_package !== selectedPackage) {
      ...
      const pkgName = PKG_NAMES[data.applies_to_package] || data.applies_to_package
      return NextResponse.json({
        success: false,
        message: `هذا الكود ينطبق فقط على باقة: ${pkgName}`,
      }, { status: 400 })
    }

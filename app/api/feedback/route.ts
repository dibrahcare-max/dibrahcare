...
// نستخدم service_role هنا عشان نتجاوز RLS على جدول feedback
// (التطبيق نفسه يتحقق من صحة البيانات في الـ POST body)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error } = await supabaseAdmin.from('feedback').insert(insert)

...
export async function POST(req: NextRequest) {
  try {
    ...
    // جلب الـ notes الحالي ودمج الملاحظة الحرّة داخله (نحافظ على بيانات الحجز JSON)
    ...
    if (!current) return NextResponse.json({ success: false }, { status: 403 })

    let meta: any = {}
    try {
      meta = (typeof current.notes === 'string' && current.notes.trim().startsWith('{'))
        ? JSON.parse(current.notes) : {}
    } catch { meta = {} }
    meta.user_note = notes || ''

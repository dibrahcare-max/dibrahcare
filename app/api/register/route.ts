...
export async function POST(req: NextRequest) {
  try {
    ...
    // ═══ رسالة ترحيب عبر واتساب (Twilio) — fire-and-forget ═══
    // لا توقف التسجيل لو فشلت — نلوّقها فقط
    const firstName = (form.full_name || '').trim().split(/\s+/)[0] || 'عميلنا الكريم'
    sendWhatsApp(form.phone, TEMPLATES.welcomeRegistration(firstName))
      .then(r => {
        if (!r.success) {
          console.warn('⚠️  [register] welcome WhatsApp failed:', r.error)
        } else {
          console.log('✅ [register] welcome WhatsApp sent:', r.sid)
        }
      })
      .catch(err => console.error('welcome WhatsApp exception:', err?.message))

    // ═══ تنبيه الأدمن بتسجيل جديد — fire-and-forget ═══
    notifyAdmins(
      TEMPLATES.adminNewRegistration(form.full_name || '—', form.phone || '—')
    )

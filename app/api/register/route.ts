...
export async function POST(req: NextRequest) {
  try {
    ...
    // ═══ رسالة ترحيب عبر واتساب + تنبيه الأدمن — ننتظرها لضمان الإرسال ═══
    const firstName = (form.full_name || '').trim().split(/\s+/)[0] || 'عميلنا الكريم'
    const welcomeSend = sendWhatsApp(form.phone, TEMPLATES.welcomeRegistration(firstName))
      .then(r => {
        if (!r.success) console.warn('⚠️  [register] welcome WhatsApp failed:', r.error)
        else console.log('✅ [register] welcome WhatsApp sent:', r.sid)
      })
      .catch(err => console.error('welcome WhatsApp exception:', err?.message))

    const adminNotify = Promise.resolve(
      notifyAdmins(TEMPLATES.adminNewRegistration(form.full_name || '—', form.phone || '—'))
    ).catch(err => console.error('admin notify exception:', err?.message))

    await Promise.allSettled([welcomeSend, adminNotify])

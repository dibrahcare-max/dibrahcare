...
// تسميات الباقات للعرض
const PKG_NAMES_DISPLAY: Record<string, string> = {
  daily_4: 'يومي ٤ ساعات', daily_8: 'يومي ٨ ساعات',
  weekly_4: 'أسبوعي ٤ ساعات', weekly_8: 'أسبوعي ٨ ساعات',
  monthly_4: 'شهري ٤ ساعات', monthly_8: 'شهري ٨ ساعات',
...
export default function DiscountCodesAdminPage() {
  ...
  const [genPackage, setGenPackage] = useState<string>('')
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/admin/list?resource=discount-codes')
      const json = await res.json()
      setCodes((json.data as any) || [])
    } catch { setCodes([]) }
    setLoading(false)
  }
    ...
    try {
      const res = await fetch('/api/admin/discount-codes/generate', {
        ...
        body: JSON.stringify({
          ...
          appliesToPackage: genPackage || null,
        }),
      })
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 24, marginBottom: 24, border: '1px solid rgba(95,97,87,.1)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 2fr auto', gap: 12, alignItems: 'flex-end' }}>
              ...
              <div>
                <label style={lblStyle}>ينطبق على باقة (اختياري)</label>
                <select value={genPackage} onChange={e => setGenPackage(e.target.value)} style={inpStyle}>
                  <option value="">كل الباقات (عام)</option>
                  <option value="daily_4">يومي ٤ ساعات</option>
                  <option value="daily_8">يومي ٨ ساعات</option>
                  <option value="weekly_4">أسبوعي ٤ ساعات</option>
                  <option value="weekly_8">أسبوعي ٨ ساعات</option>
                  <option value="monthly_4">شهري ٤ ساعات</option>
                  <option value="monthly_8">شهري ٨ ساعات</option>
                  <option value="ramadan_2">باقة رمضان</option>
                </select>
              </div>
                    ...
                    {filtered.map(c => {
                      ...
                      return (
                        <tr key={c.id} style={{ borderTop: '1px solid rgba(95,97,87,.08)' }}>
                          ...
                          <td style={tdStyle}>
                            {c.batch_label || '—'}
                            {c.applies_to_package && (
                              <div style={{ marginTop: 4 }}>
                                <span style={{ background: '#dcfce7', color: '#166534', padding: '2px 8px', borderRadius: 999, fontWeight: 700, fontSize: '.72rem', whiteSpace: 'nowrap' }}>
                                  📦 {PKG_NAMES_DISPLAY[c.applies_to_package] || c.applies_to_package}
                                </span>
                              </div>
                            )}
                          </td>

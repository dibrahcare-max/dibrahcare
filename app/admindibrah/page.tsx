...
export default function AdminPage() {
  ...
  const fetchBookings = async () => {
    ...
    const { data, error } = await supabase
      ...
      .neq('service_type', 'medical') // الحجوزات الطبية تظهر في /admindibrah/medical فقط
      .order('created_at', { ascending: false })
  ...
  const buildExportRows = () => filtered.map(b => {
    // فك الـ notes لاستخراج تفاصيل الخصم والإهداء
    let meta: any = {}
    try {
    ...
    'عمر المستفيد':       b.beneficiary_age || '',
    'صلة القرابة':        b.beneficiary_relation || '',
    'جوال طوارئ المستفيد': b.emergency_phone || '',
    ...
    'رقم التتبع':         b.track_id || '',
    'الرقم المرجعي':      b.payment_id || '',

    // ─── ملاحظات (كاملة بدون قطع) ───
...
<div class="card">
  ...
  <div class="section">
    ...
    <div class="fields">
      ...
      <div class="field"><span class="lbl">جوال الطوارئ:</span><span class="val">${esc(r['جوال طوارئ المستفيد'])}</span></div>
    </div>
  </div>
...
<div class="card">
  ...
  <div class="section">
    ...
    <table class="fields"><tr>
      ...
      <td><b>صلة القرابة:</b> ${esc(r['صلة القرابة'])}</td>
      <td><b>جوال الطوارئ:</b> ${esc(r['جوال طوارئ المستفيد'])}</td>
    </tr></table>
  </div>
...
<div class="WordSection1">
  ...
  return (
    <>
      ...
      <div style={{ background: 'var(--bg)', minHeight: '100vh', direction: 'rtl', display: 'flex', overflowX: 'hidden', maxWidth: '100vw' }}>

...
export default function MedicalAdminPage() {
  ...
  const load = async () => {
    ...
    const [regsRes, bookingsRes] = await Promise.all([
      fetch('/api/admin/list?resource=registrations-medical').then(r => r.json()).catch(() => ({ data: [] })),
      fetch('/api/admin/list?resource=bookings-medical').then(r => r.json()).catch(() => ({ data: [] })),
    ])
  ...
  const updateStatus = async (id: string, status: string) => {
    await fetch(`/api/admin/registrations/${id}/update`, {
      method: 'POST',

    ...
    fetch('/api/auth/me')
      ...
      .then(d => {
        ...
        if (d.status === 'complete') {
          ...
          const nextParam = next !== '/services' ? '&next=' + encodeURIComponent(next) : ''
  ...
  const verify = async () => {
    ...
    try {
      ...
      if (status === 'complete') {
        ...
        // جديد — يكمل بياناته في /register، ونمرر next ليرجع له بعد التسجيل
        const nextParam = next !== '/services' ? '&next=' + encodeURIComponent(next) : ''
        router.push('/register?phone=' + encodeURIComponent(phone) + nextParam)
      }
    } catch {

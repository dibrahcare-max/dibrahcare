...
function ResponseContent() {
  ...
  // ═══ منطق النجاح الصارم — مطابق ١:١ للـ PHP الأصلي اللي اشتغل لسنوات ═══
  // PHP: $success = ($result === 'CAPTURED' || $result === 'APPROVED');
  // النظر فقط على حقل result. لا paymentId fallback، لا authRespCode، لا responseCode.
  // ⚠️ Neoleap يرجع paymentId حتى في الرفض — لذلك ممنوع الاعتماد عليه كدليل نجاح.
  const resultUpper = result.toUpperCase().trim()
  const success = (resultUpper === 'CAPTURED' || resultUpper === 'APPROVED')
  ...
  useEffect(() => {
    ...
    if (!raw) {
      // ═══ تنبيه تشخيصي: نجاح لكن ما لقينا بيانات الحجز ═══
      // الأسباب الممكنة: (١) المستخدم وصل لهالصفحة مباشرة، (٢) sessionStorage ضاع بسبب
      // اختلاف origin (مثلاً www.dibrahcare.com vs dibrahcare.com)
      if (success) {
        console.warn('⚠️  [payment-response] SUCCESS but sessionStorage empty — booking NOT saved!')
        console.warn('⚠️  [payment-response] Origin:', window.location.origin)
        console.warn('⚠️  [payment-response] Likely cause: cross-origin redirect cleared sessionStorage')
      }
      return
    }
    ...
    if (success && !notified.current && booking) {
      ...
      fetch('/api/save-booking', {
        ...
        .catch(err => console.error('❌ [payment-response] save-booking fetch failed:', err))
    }

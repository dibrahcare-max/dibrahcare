...
type Support = {
  ...
  report_url?: string
  report_files?: { url: string; name?: string; uploaded_at?: string }[]
...
export default function JourneyPage({ params }: { params: { support_number: string } }) {
  ...
  // قائمة ملفات التقرير (الجديد report_files، أو القديم report_url)
  const toPreview = (url: string) =>
    url.includes('drive.google.com') ? (toDrivePreviewUrl(url) || url) : url
  const reportFilesList: { url: string; preview: string; name?: string }[] =
    (Array.isArray(support.report_files) && support.report_files.length > 0)
      ? support.report_files.map(f => ({ url: f.url, preview: toPreview(f.url), name: f.name }))
      : (support.report_url ? [{ url: support.report_url, preview: toPreview(support.report_url) }] : [])
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 20px' }}>
      ...
      {/* معاينة التقارير لو الحالة موزّعة */}
      {support.status === 'disbursed' && (reportFilesList.length > 0 || support.report_expired) && (
        <div style={{ marginTop: 36 }}>
          <h3 style={{ fontSize: '1.05rem', fontWeight: 800, color: '#2D4A1E', marginBottom: 12 }}>
            📄 {reportFilesList.length > 1 ? 'تقارير الصرف' : 'تقرير الصرف'}
          </h3>

          {support.report_expired ? (
            // التقارير انتهت مدة إتاحتها (٣ أشهر)
            <div style={{
              background: '#fef3c7', borderRadius: 12, padding: 24, textAlign: 'center',
              border: '1px solid rgba(201,168,76,.3)',
            }}>
              <div style={{ fontSize: '2rem', marginBottom: 8 }}>🗂️</div>
              <p style={{ color: '#92400e', fontWeight: 700, marginBottom: 6 }}>
                انتهت مدة إتاحة التقرير
              </p>
              <p style={{ color: 'rgba(146,64,14,.8)', fontSize: '.85rem', lineHeight: 1.7, margin: 0 }}>
                يُتاح تقرير الصرف لمدة ٣ أشهر. لطلب نسخة من التقرير، يمكنك التواصل معنا.
              </p>
            </div>
          ) : reportFilesList.length > 0 ? (
            <>
              {/* تنبيه يحثّ الداعم على حفظ نسخة */}
              <div style={{
                background: 'rgba(45,74,30,.06)', borderRadius: 10, padding: '12px 16px',
                marginBottom: 16, fontSize: '.85rem', color: '#2D4A1E', lineHeight: 1.7,
                display: 'flex', alignItems: 'flex-start', gap: 8,
              }}>
                <span style={{ fontSize: '1.1rem', flexShrink: 0 }}>📌</span>
                <span>{reportFilesList.length > 1 ? 'هذه التقارير متاحة' : 'هذا التقرير متاح'} للتحميل لمدة <strong>٣ أشهر</strong> من تاريخه. ننصحك بحفظ نسخة على جهازك للرجوع إليها لاحقاً.</span>
              </div>

              {/* الملفات فوق بعض */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                {reportFilesList.map((f, idx) => (
                  <div key={idx}>
                    {reportFilesList.length > 1 && (
                      <div style={{ fontWeight: 800, color: '#2D4A1E', fontSize: '.92rem', marginBottom: 8 }}>
                        التقرير {idx + 1}
                      </div>
                    )}
                    <div style={{
                    ...
                    <a
                      ...
                      style={{
                        ...
                        borderRadius: 10, fontWeight: 700, fontSize: '.9rem',
                      }}
                    >
                      📥 تحميل {reportFilesList.length > 1 ? `التقرير ${idx + 1}` : 'التقرير'}
                    </a>
                  </div>
                ))}
              </div>

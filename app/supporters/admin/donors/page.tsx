...
export default function DonorsListPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<Support | null>(null)
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`/api/supports/${confirmDelete.id}`, { method: 'DELETE' })
      const data = await res.json()
      if (res.ok && data.success) {
        setSupports(prev => prev.filter(s => s.id !== confirmDelete.id))
                      ...
                      <Td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                          ...
                          <button
                            ...
                            style={{
                              color: '#dc2626', fontWeight: 700, fontSize: '.85rem',
      ...
      {confirmDelete && (
        ...
        >
          ...
          >
            ...
            <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--dark)', margin: '0 0 10px' }}>
              تأكيد حذف الداعم
            </h3>
            <p style={{ color: 'rgba(95,97,87,.85)', fontSize: '.92rem', lineHeight: 1.8, margin: '0 0 8px' }}>
              سيتم حذف الداعم <strong>{confirmDelete.donor_name}</strong> (رقم {confirmDelete.support_number}) وكل ملفات تقاريره نهائياً.
            </p>
            <p style={{ color: '#dc2626', fontSize: '.85rem', fontWeight: 700, margin: '0 0 22px' }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              <button
                ...
                disabled={deleting}
                style={{
                  padding: '11px 24px', borderRadius: 10, border: '1px solid rgba(95,97,87,.25)',
                  background: 'white', color: 'var(--dark)', fontWeight: 700, fontSize: '.9rem',
              ...
              <button
                ...
                style={{
                  ...
                  background: '#dc2626', color: 'white', fontWeight: 800, fontSize: '.9rem',

...
export default function Contact() {
  ...
  const inp: React.CSSProperties = {
    width: '100%', border: '1.5px solid rgba(95,97,87,.15)',
    background: 'var(--bg)', borderRadius: 8, padding: '10px 13px',
    fontFamily: 'inherit', fontSize: '.9rem', color: 'var(--dark)', outline: 'none',
  }
  ...
  return (
    <section id="contact" style={{ padding: '96px 64px', background: 'var(--bg)' }} className="sec-contact">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, marginTop: 56 }} className="rv contact-grid">
        ...
        <div>
          {[
            ...
            { icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{width:20,height:20}}><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>, label: 'البريد', val: 'CustomerService@dibrahcare.com' },
            ...
            <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'center', padding: '14px 0', borderBottom: '1px solid rgba(95,97,87,.1)' }}>
        ...
        {/* فورم */}
        <div className="contact-form" style={{ background: '#EEEEEE', borderRadius: 16, padding: 32, border: '1px solid rgba(95,97,87,.15)' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: 24, color: 'var(--dark)', fontFamily: 'PNU, Tajawal, sans-serif' }}>أرسل رسالة</h3>
          ...
          {status === 'done' ? (
            ...
            <>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <textarea placeholder="كيف يمكننا مساعدتك؟" value={form.message} onChange={e => set('message', e.target.value)} style={{ ...inp, resize: 'vertical', minHeight: 88 }} />
              </div>
              ...
              <button onClick={send} disabled={status === 'loading'} style={{
                width: '100%', background: status === 'loading' ? '#9ca3af' : 'var(--dark)',
                color: '#F6F0D7', border: 'none', padding: 13, borderRadius: 8,
      ...
      <style>{`
        @media (max-width: 1024px) { .contact-grid { grid-template-columns: 1fr !important; gap: 40px !important; } .sec-contact { padding: 64px 24px !important; } }
        .contact-form input::placeholder, .contact-form textarea::placeholder { color: rgba(95,97,87,.45) !important; }
      `}</style>
    </section>
  )
}
